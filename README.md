# r2: Rerun commands when files change

A fast, efficient file watcher written in Rust that automatically reruns commands when files change.

## Overview

**r2** is a Rust implementation of the file watcher. It watches files for changes and automatically reruns specified commands, making it perfect for development workflows where you need to rebuild, retest, or recompile after code changes.
...
The binary will be at `target/release/r2`.

## Usage

```bash
r2 'command' [monitor_path [monitored_path...]]
```
...
```bash
r2 'make docs' *.md docs/
```

Watch multiple directories:
```bash
r2 'npm run build' src/ static/ templates/
```

## How It Works

1. **Startup**: r2 scans specified paths and runs the command immediately
...
r2 automatically ignores changes to these file types:
- Python bytecode: `.pyc`
...
```
$ r2 'echo "Building..."' src/
r2 is watching about 8 files:
src/
FIGHT!
echo "Building..."
Building...
